<script>
    ...
    /**
     * @type
     * {{id: number, subject: string, content: string, user:{username: string}, created_date: string, answers: []}[]}
     */
    let question_list = [];
    let size = 10;
    let total = 0;
    ...
    <div class="row my-3">
        ...
        <div class="col-6">
            <div class="input-group">
                ...
                <button type="button" class="btn btn-outline-secondary" on:click={() => {$keyword=kw, $page = 0}}>찾기</button>
            </div>
        </div>
    </div>
    <table class="table">
        ...
        <tbody>
            ...
            <tr class="text-center">
                ...
                <td>{moment(question.created_date).format("YYYY년 MM월 DD일 hh:mm a")}</td>
            </tr>
            {/each}
        </tbody>
    </table>
