...
def get_question_list(db: Session, skip: int = 0, limit: int = 10, keyword: str = ''):
    ...
    question_list = question_list.order_by(Question.created_date.desc())\
...
def create_question(db:Session, question_create: QuestionCreate, user: User):
    db_question = Question(subject=question_create.subject, content=question_create.content, created_date=datetime.now(), user=user)
...
def update_question(db: Session, db_question: Question, question_update: QuestionUpdate):
    ...
    db_question.modified_date = datetime.now()
    db.add(db_question)
    db.commit()
